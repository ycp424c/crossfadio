...
} from '@renderer/audio/segueVoiceGain';
...
export function PlayerView({ onAuthTokenChange, onNavigate }: PlayerViewProps): JSX.Element {
  ...
  const finishDetachedActiveSegueAudio = useCallback(
    (audio: HTMLAudioElement) => {
      activeSegueAudiosRef.current.delete(audio);
      segueVoiceGainControllerRef.current?.release(audio);
  ...
  const finishSegueAudio = useCallback(
    (pending: PendingSegueAudio, audio: HTMLAudioElement): boolean => {
      if (pendingSegueRef.current !== pending || pending.audio !== audio) {
        return false;
      }
  ...
  const configureSegueAudio = useCallback(
    (pending: PendingSegueAudio, audio: HTMLAudioElement) => {
      audio.preload = 'auto';
      audio.volume = TRACK_DEFAULT_VOLUME;
      audio.onloadedmetadata = () => {
        const duration = audio.duration;
        if (pendingSegueRef.current !== pending || pending.audio !== audio) return;
      ...
      audio.onerror = () => {
        if (finishSegueAudio(pending, audio)) {
  ...
  const disposeSegueAudio = useCallback(
    (force = false) => {
      ...
      segueVoiceGainControllerRef.current?.release(pending.audio);
  ...
  const maybeStartSegueAudio = useCallback(() => {
    ...
    void (async () => {
      ...
      if (route === 'unavailable') {
        const nativeAudioUrl = capturedAudio.src || pending.audioUrl;
        ...
        nativeAudio.volume = TRACK_DEFAULT_VOLUME;
        configureSegueAudio(pending, nativeAudio);
        pending.audio = nativeAudio;
        pending.nativeOnly = true;
        segueAudioRef.current = nativeAudio;
        capturedAudio = nativeAudio;
      }

      if (pendingSegueRef.current !== pending || pending.audio !== capturedAudio || !pending.started) {
        segueVoiceGainControllerRef.current?.release(capturedAudio);
        unloadAudioElement(capturedAudio);
        return;
      }
      ...
      try {
        for await (const { type, data } of streamSegue({
          ...
          if (type === 'segue.delta') {
            ...
            disposeSegueAudio();
            const activePending = pendingSegueRef.current;
            if (activePending?.started) {
              const activeAudio = activePending.audio;
              activeAudio.onloadedmetadata = null;
              activeAudio.onended = () => finishDetachedActiveSegueAudio(activeAudio);
              activeAudio.onerror = () => finishDetachedActiveSegueAudio(activeAudio);
            }
            ...
            configureSegueAudio(pending, audio);
  ...
  }, [configureSegueAudio, currentTrack, currentTrackId, disposeSegueAudio, finishDetachedActiveSegueAudio, maybeStartSegueAudio, nextTrack, queue]);
        ...
        <header className={`col-span-1 flex flex-col items-stretch justify-between gap-3 rounded-xl border px-4 py-3 md:col-span-12 md:flex-row md:items-center ${modeConfig.panel}`}>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <a
              ...
              className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 bg-black/20 text-zinc-300 transition hover:border-white/20 hover:bg-white/5 hover:text-zinc-100"
              href="https://github.com/ycp424c/crossfadio"
